...
function pvpivcalc(cp_cap, poke, iv_min, s_at, s_df, s_st, max_lvl, sprod, sstats){
    ...
    if(iv_min > s_at || iv_min > s_df || iv_min > s_st )
    {
        return "";
    }
    ...
    for(i=0;i<iv_combos.length;i++){
        ...
        products.push([stat_product,lvl,iv_combos[i]['at'],iv_combos[i]['df'],iv_combos[i]['st'],at,df,st]);
    }
    ...
    //ret = '<div class="row"><div class="col-2">RANK</div><div class="col-1">ATK</div><div class="col-1">DEF</div><div class="col-1">HP</div><div class="col-2">LV</div><div class="col-2">CP</div><div class="col-2"> % '+(sprod?' (product)':'')+(sstats?' (at, df, st)':'')+'</div></div>';
    
    
    name_league = "Little League";

    if (cp_cap == 1500)
    {
        name_league = "Super League";
    }
    else if (cp_cap == 2500)
    {
        name_league = "Ultra League";
    } else if (cp_cap == 999999)
    {
        name_league = "Master League";
    }

    ret = '<div class="row">&nbsp;' + poke['name'] + ' - ' + name_league + '</div>';
    ret += '<div class="row" style="border-top: 1px black solid; background-color: hsl('+Math.round(90*p_scale)+',100%,50%)"><div class="col-2" style="color: black;">'+rank+'</div><div class="col-1" style="color: black;">'+s_at+'</div><div class="col-1" style="color: black;">'+s_df+'</div><div class="col-1" style="color: black;">'+s_st+'</div><div class="col-2" style="color: black;">'+(lvl/2+1)+'</div><div class="col-2" style="color: black;">'+cp(at,df,st,cpm2[lvl])+'</div><div class="col-2" style="color: black;">'+Math.round(stat_product/products[0][0]*100000)/1000+''+(sprod?' ('+Math.round(stat_product)+')':'')+(sstats?' ('+Math.round(cpm[lvl]*at*100)/100+', '+Math.round(cpm[lvl]*df*100)/100+', '+Math.floor(cpm[lvl]*st)+')':'')+'</div></div>';
    for (i=0;i<3;i++){
        ret += '<div class="row" style="border-top: 1px black solid;"><div class="col-2">'+(i+1)+'</div><div class="col-1">'+products[i][2]+'</div><div class="col-1">'+products[i][3]+'</div><div class="col-1">'+products[i][4]+'</div><div class="col-2">'+(products[i][1]/2+1)+'</div><div class="col-2">'+cp(Math.floor(poke['at'])+products[i][2],Math.floor(poke['df'])+products[i][3],Math.floor(poke['st'])+products[i][4],cpm2[products[i][1]])+'</div><div class="col-2">'+Math.round(products[i][0]/products[0][0]*100000)/1000+''+(sprod?' ('+Math.round(products[i][0])+')':'')+(sstats?' ('+Math.round(cpm[products[i][1]]*products[i][5]*100)/100+', '+Math.round(cpm[products[i][1]]*products[i][6]*100)/100+', '+Math.floor(cpm[products[i][1]]*products[i][7])+')':'')+'</div></div>';
    }
